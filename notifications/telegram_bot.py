...
def format_signal_alert(signal, label: str) -> str:
    ...
    text = (
        ...
        f"{direction_emoji} 资产： *{asset_display}*\n"
        f"方向： *{signal.direction}*\n\n"
        f"进场价：       `{signal.entry:.6f}`\n"
        f"止损价：   `{signal.stop_loss:.6f}`\n"
        f"止盈价： `{signal.take_profit:.6f}`\n"
        f"盈亏比： *{signal.rr:.2f}*\n\n"
        f"原始评分：   *{signal.raw_score:.0f}/10*\n"
        f"最终评分： *{signal.final_score:.0f}/10*\n"
...
def format_alert(setup: dict, score: int, label: str) -> str:
    ...
    text = (
        ...
        f"止损价：   `{setup['stop_loss']:.6f}`\n"
        f"止盈价： `{setup['take_profit']:.6f}`\n"
        f"盈亏比： *{setup['rr']:.2f}*\n\n"
        f"回调： {pullback_str}\n"
...
def format_zone_signal_alert(signal, label: str) -> str:
    ...
    text = (
        ...
        f"止盈价： `{fp(signal.take_profit)}`\n"
        f"盈亏比： *{signal.rr:.2f}*\n\n"
        f"原始评分：   *{signal.raw_score:.0f}/11*\n"
        f"最终评分： *{signal.final_score:.0f}/11*\n\n"
        f"H4 偏向： {bias_h4}\n"
        f"区域： `{fp(zone_level)}` ({zone_touches} 次触碰)\n"
        f"ATR(日)： `{fp(atr_daily)}`\n"
        f"宏观风险： {macro_risk}\n"
        f"时段： {session}\n"
        f"动量： {momentum_arrow} {momentum}\n"
        f"形态： {pattern}"
    )
