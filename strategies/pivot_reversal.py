...
Pivot Reversal V1.0 — con diagnostica dettagliata
"""
...
class PivotReversal(BaseStrategy):
    ...
    def _at_level(self, candle, lp, atr_val, direction):
        return (abs(float(candle["low"]) - lp) <= 0.5 * atr_val
                if direction == "LONG"
                else abs(float(candle["high"]) - lp) <= 0.5 * atr_val)

    def _score(self, trend_h4, trend_h1, at_level, rejection, trigger):
        score = 0
        if trend_h4:  score += 3
        if trend_h1:  score += 2
        if at_level:  score += 2
    ...
    def generate_signal_with_diagnostics(self, market_data: dict) -> Tuple[Optional[Signal], dict]:
        ...
        trigger_ok = (float(c_trigger["close"]) > float(c_rejection["high"])
                      if direction == "LONG"
                      else float(c_trigger["close"]) < float(c_rejection["low"]))

        diag["conditions"]["Prezzo al livello (<=0.5 ATR)"] = bool(at_level)
        diag["conditions"]["Rejection candle (wick>=2x corpo)"] = bool(rejection_ok)
        diag["conditions"]["Trigger H1"] = bool(trigger_ok)

        if not at_level:
            diag["rejection_reason"] = "PRICE_NOT_AT_LEVEL"
            return None, diag
        if not rejection_ok:
        ...
        raw_score = float(self._score(trend_h4, trend_h1, at_level, rejection_ok, trigger_ok))
        ...
        signal = Signal(
            ...
            additional_context={
                ...
                "level_price": lp, "at_level": at_level,
